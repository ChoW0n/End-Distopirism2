...
### 6.5.1 2.5D 투영 — FOV 를 새로 정하지 않는다

`assets/map/placement.json` 이 이미 들고 있는 값으로만 푼다.

| 값 | 쓰임 |
|---|---|
| `groundBoundaryApprox` 0.445 | 지평선. 여기에 가까울수록 작아진다 |
| `footYRatio` 0.76 | 발 기준선. 배율 1 이 되는 높이 |
| `characterHeightRatio` 0.3 | 캐릭터 키가 화면에서 차지할 비율 |
| 레이어별 `parallax` | 배경이 카메라를 얼마나 따라가는가 |

```
무대 1px 당 화면 px  = characterHeightRatio × 화면높이 / H
화면 y               = 발기준선 + (무대 y − 접지선) × 단위
원근 배율            = (화면 y − 지평선) / (발기준선 − 지평선)
```
...
| 명령 | 붙드는 시간 |
|---|---|
| `playFrames` (여러 장) | 장 수 × 130ms |
| `playFrames` (한 장) | 0.18초 |
| `dashTo` · `dashBack` | 목적지까지 걸리는 시간 |
| `ultimate` (cutscene) | 컷신 길이 — **타임라인 장벽** |
| `ultimate` (그 외) | 0.25초 |
| `spawnEffect` · `bar` · `clashBadge` · `targetArrow` | 0 |
...
| 항목 | 현황 |
|---|---|
| 프레임 재생 시간 | **렌더러가 장당 130ms 로 균등 재생한다.** 매니페스트에 장별 시간이 없다 |
| 명중점 최종값 | `hitPoint` 는 접지점·머리 중간을 쓰는 중. 눈으로 보고 다시 잡는다 |
| 확대 배율 | `FOCUS_ZOOM 1.6` 은 임시값 |
| 컷신 길이 | 1.6초 고정. 연출 확정되면 데이터로 뺀다 |
