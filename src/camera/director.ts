...
//교전에 붙었을 때의 확대 배율. 스프라이트가 붙으면 눈으로 보고 다시 잡는다
const FOCUS_ZOOM = 1.6;

//이 피해량이면 흔들림이 최대가 된다는 기준값
const SHAKE_REFERENCE_DAMAGE = 30;
...
//카메라가 지금 보고 있는 것. 이 값이 카메라 상태의 유일한 출처다
export type CameraShot =
  | { kind: 'idle' }
  | { kind: 'focus'; subjectIds: [string, string]; zoom: number };
...
export class CameraDirector {
  ...
  consume(events: readonly BattleEvent[]): CameraCommand[] {
    ...
    for (let i = 0; i < events.length; i += 1) {
      ...
      switch (event.type) {
        ...
        case 'damageApplied':
          if (event.damage > 0) {
            commands.push({ type: 'shake', intensity: Math.min(1, event.damage / SHAKE_REFERENCE_DAMAGE) });
          }
          break;
        ...
        case 'clashEnd':
        case 'oneSidedEnd':
          if (!events.slice(i + 1).some(isEngagementStart)) {
  ...
  private pushShot(commands: CameraCommand[], next: CameraShot): void {
    ...
    commands.push(
      next.kind === 'idle'
        ? { type: 'idle' }
        : { type: 'focus', subjectIds: next.subjectIds, zoom: next.zoom },
    );
  }
